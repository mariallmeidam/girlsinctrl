...
// ------ Forget ------
Future<ApiResponse> forget(String email) async {
  ApiResponse apiResponse = ApiResponse();
  try {
    final response = await http.post(Uri.parse(forgetURL), headers: {
      'Accept': 'application/json'
    }, body: {
      'email': email,
    });
...
Future<ApiResponse> reset(
  ...
  ApiResponse apiResponse = ApiResponse();
  try {
    final response = await http.post(Uri.parse(resetURL), headers: {
      'Accept': 'application/json'
    }, body: {
      'email': email,
      'token': codigo,
